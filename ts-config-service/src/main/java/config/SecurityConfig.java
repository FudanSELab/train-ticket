...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity httpSecurity) throws Exception {
        httpSecurity.httpBasic().disable()
                ...
                .and()
                .authorizeRequests()
                .antMatchers("/api/v1/config/admin/**").hasRole(admin)
                .antMatchers("/api/v1/config/**").permitAll()
                .antMatchers(HttpMethod.POST, "/api/v1/config/configs").hasAnyRole(admin)
                .antMatchers(HttpMethod.PUT, "/api/v1/config/configs").hasAnyRole(admin)
                .antMatchers(HttpMethod.DELETE, "/api/v1/config/configs/*").hasAnyRole(admin)
